...
static int __init bitSurfing_Receiver_init(void){
  ...
  */
  ...
  long long int *addedDelay, delayToIntroduce, BitForLoopProcessingTime = 0;
  ...
  addedDelay = (long long int *)kmalloc(70*sizeof(long long int), GFP_KERNEL);
  ...
  if(byteOfMsg && addedDelay){
    for(i=0;i<70;i++)
      printk(KERN_INFO "Msg found in Byte : %hu | %lld | %lld\n", byteOfMsg[i], myByteFlagTimes[i], addedDelay[i]);
  }
