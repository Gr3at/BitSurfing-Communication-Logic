...
#define MAXRECVSTRING 1000 /* Longest string to receive */
...
#define OUTPIN 2 // board pin P8.7
#define INPIN 5 // board pin P8.9
...
int main(void)
{
    ...
    /* if program blows here it need to export gpio pins in sys/class/gpio */
    ...
    while((successCounter<SUCCESSES_TO_FINISH) && (successReceivedCounter<SUCCESSES_TO_FINISH)){
        ...
        for(int byte=100; byte<MAXRECVSTRING; byte+=100){
