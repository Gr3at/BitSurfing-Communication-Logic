...
#define SECONDSTOWAITBEFOREBREAK 600 /* aka 10mins */ /* set to something very big if only sending data */
...
static int __init bitSurfing_Receiver_init(void){
  ...
  */
  ...
  size_t x_bits_till_next_msg = 0, packetCounter=0, forLoopCounter=0;
  ...
  ktime_t foundMsgTime=ktime_get(), prevFoundMsgTime, NoMsgBreak=ktime_get(), sendMsgTime=ktime_get(),prevSendMsgTime;
  ktime_t prePacketTime, afterPacketTime, proccessingTime, afterProccessingTime, startForLoopTimer;
  ...
  if((meanDataPtr==NULL) || (meanSendMsgDataPtr==NULL) || (meanPacketPtr==NULL) || (meanProccessingPtr==NULL))
  ...
  while((successCounter<SUCCESSES_TO_FINISH) && (successReceivedCounter<SUCCESSES_TO_FINISH)){
    ...
    for(byte=PACKET_HEADERS_SIZE_BEGIN; byte<MAXRECVSTRING; byte+=READING_STEP){
      ...
      if(forLoopCounter<DATA2COLLECT)
        ProccessingForLoopInterrarivalTime[forLoopCounter] = ktime_to_ns(ktime_sub(ktime_get(),startForLoopTimer));
      forLoopCounter++;
    }
  ...
  //struct file *sendingfd = kopen("sendBBB.txt", O_CREAT | O_RDWR | O_TRUNC, S_IRGRP | S_IWGRP);
  //struct file *receivingfd = kopen("recBBB.txt", O_CREAT | O_RDWR | O_TRUNC, S_IRGRP | S_IWGRP);
  ...
  snprintf(logBuffer, 100, "| %zu | %lld | %zu | %lld | %zu | %lld | %zu | %lld\n", meanDataPtr->samplesNum, meanDataPtr->mean, meanSendMsgDataPtr->samplesNum, meanSendMsgDataPtr->mean, meanPacketPtr->samplesNum, meanPacketPtr->mean, meanProccessingPtr->samplesNum, meanProccessingPtr->mean);
