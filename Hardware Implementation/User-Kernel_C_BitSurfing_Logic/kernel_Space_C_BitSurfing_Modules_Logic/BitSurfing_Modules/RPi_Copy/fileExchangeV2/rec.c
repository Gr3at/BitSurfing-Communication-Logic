...
#define READING_STEP 333 /* 1000, 500, 333, 250, 200, 166, 142, 125, 111, 100, 90, 83, 76, 71, 66, 62, 58, 55, 52, 50 */
...
#define CPUFREQUENCY 700 /* in kiloHertz */
...
static int __init bitSurfing_Receiver_init(void){
  ktime_t moduleInTime = ktime_get();
  ...
  */
  ...
  unsigned short receivedMsgs[SUCCESSES_TO_FINISH], oldReceivedMsgs[SUCCESSES_TO_FINISH], afterReceivedMsgs[SUCCESSES_TO_FINISH], after2ReceivedMsgs[SUCCESSES_TO_FINISH];
  bool myByteFlag = false, cacheNextMsg = false, terminationFlag=false, cachOneMoreMsg = false;
  size_t myByteFlagCounter = 0, x_bits_till_next_msg = 0, packetCounter=0, forLoopCounter=0, byteCounter;
  ...
  long long int delayToIntroduce, BitForLoopProcessingTime;
  ktime_t foundMsgTime=ktime_get(), prevFoundMsgTime, NoMsgBreak=ktime_get(), prevRecMsgTime, recMsgTime=ktime_get();
  ...
  if(ProccessingForLoopInterrarivalTime==NULL || PacketInterrarivalTime==NULL || ProccessingInterrarivalTime==NULL)
    return(-1);
  ...
  meanData *meanRecMsgDataPtr = (meanData *)kmalloc(sizeof(meanData), GFP_KERNEL);
  ...
  if((meanDataPtr==NULL) || (meanRecMsgDataPtr==NULL) || (meanPacketPtr==NULL) || (meanProccessingPtr==NULL))
    printk(KERN_ERR "Failed to allocate memmory for meanDataPtr.\n");
  meanDataPtr->samplesNum = 0;
  meanRecMsgDataPtr->samplesNum = 0;
  ...
  while((successCounter<SUCCESSES_TO_FINISH) && (successReceivedCounter<SUCCESSES_TO_FINISH)){
    ...
    if(meanRecMsgDataPtr->samplesNum>=70/*(terminationFlag==true) || */ /* (ktime_after(foundMsgTime, NoMsgBreak) */ /* if foundMsgTime > NoMsgBreak - ensure positive sub */ /* && (ktime_ms_delta(foundMsgTime, NoMsgBreak)>((long long int ) (SECONDSTOWAITBEFOREBREAK*1000))) )*/){
      printk(KERN_INFO "broke because no message for %lld mseconds.\n", ktime_ms_delta(foundMsgTime, NoMsgBreak));
      break;
    }

    proccessingTime = ktime_get();
    ...
    for(byte=PACKET_HEADERS_SIZE_BEGIN; byte<MAXRECVSTRING; byte+=READING_STEP){
      ...
      for (bits = 7; bits >= 0; bits--){
        ...
        if((foundMsg = searchValidMsgInBuffer(buffer>>bits))!=0){
          ...
          if(cacheNextMsg){
            ...
            if(foundMsg==TERMINATION_MESSAGE){
              ...
              printk(KERN_INFO "Set flag to TRUE!!\n");
              break;
            }
          }
        }
        if(gpioEventDetected()){
          myByteFlag = true;
          NoMsgBreak = ktime_get(); /* if no message arrived for long time then break */
          /* timing related and send messages number tracking */
          prevRecMsgTime = recMsgTime;
          recMsgTime = ktime_get();
          meanCalc(meanRecMsgDataPtr, ktime_ms_delta(recMsgTime,prevRecMsgTime));
          byteOfMsg[successReceivedCounter] = byteCounter;
          /* ---------------------------------------------- */
      ...
      //delayToIntroduce = ktime_to_ns(ktime_add_ns(ktime_get(),mydiv(byteCounter*CPUFREQUENCY,BitForLoopProcessingTime+1)));
      //while(ktime_to_ns(ktime_get()) < delayToIntroduce);
      //  printk(KERN_INFO "%lld , %lld \n", ktime_to_ns(ktime_get()), delayToIntroduce);
      /* ---------------------- */
      byteCounter++;      
    }
    afterProccessingTime = ktime_get();
    if(packetCounter<DATA2COLLECT)
  ...
  if(meanRecMsgDataPtr)
    printk(KERN_INFO "Total number of received Messages count = %zu ,providing a rolling average of %lld mseconds between messages of interest.\n",meanRecMsgDataPtr->samplesNum, meanRecMsgDataPtr->mean);
  ...
  if(byteOfMsg){
    for(i=0;i<70;i++)
      printk(KERN_INFO "Msg found in Byte : %hu | %lld\n", byteOfMsg[i], myByteFlagTimes[i]);
  }

  snprintf(logBuffer, 100, "| %zu | %lld | %zu | %lld | %zu | %lld | %zu | %lld\n", meanDataPtr->samplesNum, meanDataPtr->mean, meanRecMsgDataPtr->samplesNum, meanRecMsgDataPtr->mean, meanPacketPtr->samplesNum, meanPacketPtr->mean, meanProccessingPtr->samplesNum, meanProccessingPtr->mean);
  ...
  for(i=0;i<successReceivedCounter;i++){
    snprintf(logBuffer, 100, "%hu | %hu | %hu | %hu | %zu\n", receivedMsgs[i], oldReceivedMsgs[i], afterReceivedMsgs[i], after2ReceivedMsgs[i], x_bits_til_next_valid_msg[i]);
  ...
  long long int moduleTotalTime = ktime_to_ms(ktime_sub(ktime_get(), moduleInTime)); /* equivalent to ktime_ms_delta(ktime_get(),moduleInTime) */
  printk(KERN_INFO "Module took %lld mseconds to finish.\n", moduleTotalTime);
  return(0);
}
