...
static int __init bitSurfing_Receiver_init(void){
  ...
  */
  ...
  long long int delayToIntroduce, BitForLoopProcessingTime, cpuDivisionDiff, availableTimeSlot, timePerByte;
  ktime_t foundMsgTime=ktime_get_real(), prevFoundMsgTime, NoMsgBreak=ktime_get_real(), prevRecMsgTime, recMsgTime=ktime_get_real();
  ...
  if((meanDataPtr==NULL) || (meanRecMsgDataPtr==NULL) || (meanPacketPtr==NULL) || (meanProccessingPtr==NULL) || (meanProccessingForLoopPtr==NULL))
    ...
    for(byte=PACKET_HEADERS_SIZE_BEGIN; byte<MAXRECVSTRING; byte+=READING_STEP){
      ...
      for (bits = 7; bits >= 0; bits--){
        ...
        if(gpioEventDetected()){
          myByteFlag = true;
          NoMsgBreak = ktime_get_real(); /* if no message arrived for long time then break */
          /* timing related and send messages number tracking */
          prevRecMsgTime = recMsgTime;
          recMsgTime = ktime_get_real();
      ...
      if(cpuDivisionDiff>0)
        ...
        delayToIntroduce = ktime_to_ns(ktime_get_real()); /* add no delay practicaly */
      ...
      forLoopCounter++;
      /* introduce Xnsec delay */
      //delayToIntroduce = ktime_to_ns(ktime_add_ns(ktime_get_real(),mydiv(byteCounter*CPUFREQUENCY,BitForLoopProcessingTime+1)));
      //while(ktime_to_ns(ktime_get_real()) < delayToIntroduce);
      //  printk(KERN_INFO "%lld , %lld \n", ktime_to_ns(ktime_get_real()), delayToIntroduce);
      /* ---------------------- */
      byteCounter++;
  ...
  */
  ...
  printk(KERN_INFO "| %zu | %lld | %zu | %lld | %lld | %zu | %lld | %zu | %lld\n", meanDataPtr->samplesNum, meanDataPtr->mean, meanRecMsgDataPtr->samplesNum, meanRecMsgDataPtr->mean, moduleTotalTime, meanPacketPtr->samplesNum, meanPacketPtr->mean, meanProccessingPtr->samplesNum, meanProccessingPtr->mean);
